...
public:
	...
	DECLARE_ATTRIBUTE_PTR_GI(Plan, dH::Plan);

	// getter for sub-plans
	dH::Plan *GetPlan(int nLevel);
	...
	//		to next
	void InvFiltIntensityMap(int nLevel, const dH::IntensityMapType * vWeights,
								dH::IntensityMapType * vFiltWeights);

protected:
	// array of plans (= plan pyramid)
	vector<dH::Plan::Pointer> m_arrPlans;

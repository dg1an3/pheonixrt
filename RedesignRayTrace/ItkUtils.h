...
template<int DIM> inline
itk::Offset<DIM>
	MakeOffset(itk::ContinuousIndex<REAL,DIM> index)
{
	itk::Offset<DIM> offset;
	for (int nC = 0; nC < DIM; nC++) offset[nC] = Round<int>(index[nC]);
	return offset;
}
...
template<> INLINE 
bool 
	DivVoxels(Ipp32f *pVoxelsLDiv, int nWidthLDiv,
			const Ipp32f *pVoxelsR, int nWidthR,
			const itk::Size<3>& vSize)
{
